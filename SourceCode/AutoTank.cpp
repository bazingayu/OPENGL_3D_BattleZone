...
{
    //make the auto tank point at my tank
...
//	float radGunAngle = M_PI*15.0/180.0;
    ...
    uniform_real_distribution<double> u(10.0,50.0);
...
//	bombStartPos.setZ(currentPosition.Z()- 14.0*cos(radGunAngle)*sin(radTankAngle));
    ...
    std::cout <<"floats = "<< s << " floath= " << h << " floatx = " << x << " bombvelocitymag= " << bombVelocityMag << std::endl;
    ...
    std::cout << "bomb velocity" << " " << bombVelocity.X() << " " << bombVelocity.Y() << " " << bombVelocity.Z() << std::endl;
